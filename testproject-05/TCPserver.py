...
class TCP_server:
    ...
    def get_client(self):
        while True:
            ...
            try:
                ...
                server_info = '[系统消息' + ' ' + time.strftime("%H:%M:%S") + f']   消息发送失败，与客户端的连接已断开。  |错误描述:{error}'
                self.write_logs(server_info, '')
            else:
                try:
                    #  启动线程
                    ...
                    self.clients.remove(client)
                    client.close()
                    server_info = '[系统消息' + ' ' + time.strftime("%H:%M:%S") + f']   线程未能启动，并与客户端的连接已断开。  |错误描述:{error}'
                    self.write_logs(server_info, '')
    ...
    def get_msg(self, client, clients, clients_name_ip, address):
        try:
            ...
            if '#' not in name:
                ...
                server_info = '[系统消息' + ' ' + time.strftime("%H:%M:%S") + ']   客户端 ' + str(address[ 0 ]) + ':' + str(address[ 1 ]) + f' 创建用户名为：[{name}]'
                self.write_logs(server_info, '')
            else:
                uname = '未创建用户名'
                ...
                client.send(recvinfo.encode())
                server_info = '[系统消息' + ' ' + time.strftime("%H:%M:%S") + ']   客户端 ' + str(address[ 0 ]) + ':' + str(address[ 1 ]) + f' 因创建用户名违规，用户名更改为：[未命名用户{len(clients)}]'
                self.write_logs(server_info, '')
        except Exception as error:
            self.clients.remove(client)
            server_info = '[系统消息' + ' ' + time.strftime("%H:%M:%S") + f']   消息发送失败，未收到对方创建的昵称，与客户端的连接已断开。   |错误描述:{error}'
            self.write_logs(server_info, '')
        # else:
        ...
        while True:
            ...
            try:
                ...
                server_info = '[系统消息' + ' ' + time.strftime("%H:%M:%S") + f']   消息接收失败，与客户端 {address[ 0 ]}:{address[ 1 ]} 的连接已断开。   |错误描述:{error}'
                self.write_logs(server_info, '')
                self.close_client(client, address)
                break
            #  聊天记录写入到文件
            try:
                info = '[系统消息' + ' ' + time.strftime("%H:%M:%S") + ']   ' + f'客户端 {address[ 0 ]}:{address[ 1 ]} 发送了一条消息。   |key:{self.makekey()}'
    ...
    def close_client(self, client, address):
        if client not in self.clients:
            ...
            server_info = '[系统消息' + ' ' + time.strftime("%H:%M:%S") + ']   ' + '[' + self.clients_name_ip[
                address ] + ']' + ' 已经离开'
            self.write_logs(server_info, '')
        ...
        while True:
            ...
            for i in clients:
                ...
                try:
                    ...
                    server_info = '[系统消息' + ' ' + time.strftime("%H:%M:%S") + ']   ' + f'在线用户信息消息发送失败    |错误描述：{error}'
                    self.write_logs(server_info, '')
                    break
